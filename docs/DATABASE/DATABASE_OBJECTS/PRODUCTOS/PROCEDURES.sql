...
BEGIN
    IF METHOD = "LISTADO_NORMAL" THEN
		SELECT NOMBRE, CODIGO FROM productos WHERE ESTADO = 1 ORDER BY NOMBRE;
	ELSEIF METHOD = "LISTADO_INVENTARIO" THEN
		SELECT ID, NOMBRE, CODIGO, UNIDAD_MEDIDA FROM productos where ESTADO = 1 ORDER BY NOMBRE;
	END IF;
END;
DELIMITER ;
