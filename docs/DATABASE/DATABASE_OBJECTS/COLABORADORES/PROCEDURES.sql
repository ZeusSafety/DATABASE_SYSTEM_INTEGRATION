...
BEGIN
    IF method = "LISTADO_COLABORADORES_PERMISOS" THEN
		SELECT C.ID_PERSONA,C.NOMBRE, C.APELLIDO, A.NOMBRE AS AREA, C.ESTADO, C.USUARIO
		FROM colaboradores as C
        INNER JOIN AREAS AS A
        ON C.AREA_PRINCIPAL = A.ID
        WHERE CULMINACION = "ACTUALMENTE" AND C.ESTADO IN (1,0) ORDER BY ID_PERSONA ASC
        ;
    ELSEIF method = "COLABORADORES" THEN
		SELECT ID_PERSONA,NOMBRE,AREA
		FROM colaboradores WHERE CULMINACION = "ACTUALMENTE" AND ESTADO=1 ORDER BY ID_PERSONA ASC;
	ELSEIF method = "PERFIL_COLABORADOR" THEN
		...
		FECHA_NACIMIENTO, FECHA_INGRESO, FECHA_PLANILLA, USUARIO, CONTRASEÑA2, CORREO,
    ...
    ELSEIF method = "FECHA_NACIMIENTOS_CALENDARIO" THEN
		...
		ON C.AREA_PRINCIPAL = A.ID
        WHERE C.ESTADO = 1
        AND CULMINACION = "ACTUALMENTE" AND C.ID_PERSONA != 36
        ORDER BY MONTH(FECHA_NACIMIENTO) ASC;
    ELSEIF METHOD = "ASESORES_PAGINA_ESTATICA" THEN
		SELECT C.NOMBRE, SEGUNDO_NOMBRE, APELLIDO, SEGUNDO_APELLIDO, I.IMAGE_URL, 
		...
		ON C.ID_PERSONA = M.ID_COLABORADOR
		WHERE I.ESTADO=1 AND I.TIPO = "FOTOCHECK"
		AND C.AREA_PRINCIPAL = 8
        GROUP BY C.NOMBRE, SEGUNDO_NOMBRE, APELLIDO, SEGUNDO_APELLIDO, I.IMAGE_URL ;
    END IF;
END $$;
DELIMITER ;
CALL COLABORADORES2026("PERFIL_COLABORADOR", "dayanaZeus");
CALL COLABORADORES2026("FECHA_NACIMIENTOS_CALENDARIO", "random");
CALL COLABORADORES2026("ASESORES_PAGINA_ESTATICA", "random");
CALL COLABORADORES2026("LISTADO_COLABORADORES_PERMISOS", "random");
