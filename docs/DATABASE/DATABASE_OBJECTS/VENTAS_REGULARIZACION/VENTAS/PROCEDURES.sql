...
BEGIN
	...
	ELSEIF METHOD = "VENTAS_REGIONES" THEN
		SELECT REGION,SUM(TOTAL) AS TOTAL FROM VENTAS_POWER_BI
		GROUP BY REGION;
    END IF;
END $$;
DELIMITER ;
...
CALL METRICAS("VENTAS_REGIONES","random");
...
WITH NUMEROS AS(
select ID_VENTA, if(left(N_COMPROBANTE,1)="F",substring(N_COMPROBANTE,2),Null) AS NUMBER from ventas
order by ID_VENTA DESC
limit 20
)
SELECT * FROM NUMEROS
...
SELECT * FROM detalle_regularizacion 
ORDER BY FECHA_REGULARIZACION desc
limit 30;


############################################################
-- QUERY PARA SOLUCIONAR LAS REGULARIZACIONES B
SET SQL_SAFE_UPDATES = 0;
WITH CORRECCION AS (
	SELECT ID_DETALLE, COMPROBANTES, LEFT(COMPROBANTES, 2) as DIF FROM detalle_regularizacion 
	ORDER BY FECHA_REGULARIZACION desc
	limit 500
),
NECESARIOS AS(
SELECT * FROM CORRECCION WHERE DIF LIKE "%B%"
)
UPDATE detalle_regularizacion SET COMPROBANTES = CONCAT("F", " " ,SUBSTRING(COMPROBANTES,2, 9))
WHERE ID_DETALLE IN (SELECT ID_DETALLE FROM NECESARIOS WHERE DIF !="B ");

SET SQL_SAFE_UPDATES = 1;
############################################################
-- QUERY PARA SOLUCIONAR LAS REGULARIZACIONES F
SET SQL_SAFE_UPDATES = 0;
WITH CORRECCION AS (
	SELECT ID_DETALLE, COMPROBANTES, LEFT(COMPROBANTES, 1) as DIF FROM detalle_regularizacion 
	ORDER BY FECHA_REGULARIZACION desc
	limit 500
)
UPDATE detalle_regularizacion SET COMPROBANTES = CONCAT("F", " " ,SUBSTRING(COMPROBANTES,2, 9))
WHERE ID_DETALLE IN (SELECT ID_DETALLE FROM CORRECCION WHERE DIF = "F");
;
SET SQL_SAFE_UPDATES = 1;

############################################################
-- QUERY PARA SOLUCIONAR LAS REGULARIZACIONES NV
SET SQL_SAFE_UPDATES = 0;
WITH CORRECCION AS (
	SELECT ID_DETALLE, COMPROBANTES, LEFT(COMPROBANTES, 2) as DIF FROM detalle_regularizacion 
	ORDER BY FECHA_REGULARIZACION desc
...
UPDATE detalle_regularizacion SET COMPROBANTES = CONCAT("P", " " ,SUBSTRING(COMPROBANTES,3, 9)) 
WHERE ID_DETALLE IN (SELECT ID_DETALLE FROM CORRECCION WHERE DIF = "NV");
;
SET SQL_SAFE_UPDATES = 1;
...
SET SQL_SAFE_UPDATES = 0;
WITH CORRECCION AS (
	SELECT ID_DETALLE, COMPROBANTES, LEFT(COMPROBANTES, 3) as DIF FROM detalle_regularizacion 
	ORDER BY FECHA_REGULARIZACION desc
	limit 500
)
UPDATE detalle_regularizacion SET COMPROBANTES = CONCAT("F", " ", SUBSTRING(COMPROBANTES,4, 10))
WHERE ID_DETALLE IN (SELECT ID_DETALLE FROM CORRECCION WHERE DIF = "F  ")
;
SET SQL_SAFE_UPDATES = 1;

SELECT ID_DETALLE, COMPROBANTES, DIF, CONCAT("F", " ", SUBSTRING(COMPROBANTES,4, 10)) AS SOLUCION FROM CORRECCION
WHERE DIF = "F  ";