...
CALL PERMISOS('SUBVISTAS','eliaszeus');
...
DROP PROCEDURE IF EXISTS ELIMINAR_INSERTAR_SUB_VISTA_2026;
DELIMITER $$;
CREATE PROCEDURE ELIMINAR_INSERTAR_SUB_VISTA_2026(IN METHOD VARCHAR(30), IN IN_USUARIO VARCHAR(30), IN IN_ID_SUB_VISTA INT)
BEGIN
	-- Declaramos una variable para filtrar
    DECLARE FILTRO INT;
	IF METHOD = "AÑADIR_SUB_VISTA" THEN
		SELECT ID INTO FILTRO FROM SUB_VISTAS_COLABORADORES
        WHERE ID_COLABORADOR = (SELECT ID_PERSONA FROM colaboradores WHERE USUARIO = IN_USUARIO LIMIT 1) 
        AND ID_SUB_VISTAS = IN_ID_SUB_VISTA
        LIMIT 1;
        IF FILTRO IS NULL THEN
			INSERT INTO SUB_VISTAS_COLABORADORES(ID_COLABORADOR, ID_SUB_VISTAS) VALUES (
            ((SELECT ID_PERSONA FROM colaboradores WHERE USUARIO = IN_USUARIO LIMIT 1) ), IN_ID_SUB_VISTA);
		END IF;
    ELSEIF METHOD = "ELIMINAR_SUB_VISTA" THEN
		SET SQL_SAFE_UPDATES = 0;
		DELETE FROM SUB_VISTAS_COLABORADORES WHERE ID_COLABORADOR = (SELECT ID_PERSONA FROM colaboradores WHERE USUARIO = IN_USUARIO LIMIT 1)
        AND ID_SUB_VISTAS = IN_ID_SUB_VISTA;
        SET SQL_SAFE_UPDATES = 1;
    END IF;
END $$;
DELIMITER ;
select * from SUB_VISTAS_COLABORADORES WHERE ID_COLABORADOR = 19;

SET SQL_SAFE_UPDATES = 0;
DELETE FROM SUB_VISTAS_COLABORADORES WHERE ID_COLABORADOR = 19
AND ID_SUB_VISTAS = 31;
SET SQL_SAFE_UPDATES = 1;

CALL ELIMINAR_INSERTAR_SUB_VISTA_2026("AÑADIR_SUB_VISTA","eliaszeus", 30);
...
select * from colaboradores where nombre like "%admin%";

SET SQL_SAFE_UPDATES = 0;
UPDATE MODULOS SET ESTADO = 0 WHERE ID_COLABORADOR = 19 AND ID_AREA = 3;
SET SQL_SAFE_UPDATES = 1;

SELECT * FROM MODULOS 
WHERE ID_COLABORADOR = 19
limit 30;

SELECT ID_PERSONA, NOMBRE, CONTRASEÑA2, CONTRASEÑA,USUARIO FROM colaboradores 
WHERE ESTADO = 1
ORDER BY ID_PERSONA DESC 
;

DROP PROCEDURE IF EXISTS ACTUALIZAR_MODULOS;
DELIMITER $$;
CREATE PROCEDURE ACTUALIZAR_MODULOS(IN IN_ESTADO INT, IN_ID_COLABORADOR INT, IN IN_ID_AREA INT)
BEGIN
	SET SQL_SAFE_UPDATES = 0;
	UPDATE MODULOS SET ESTADO= IN_ESTADO WHERE ID_COLABORADOR=IN_ID_COLABORADOR AND 
    ID_AREA = IN_ID_AREA;
    SET SQL_SAFE_UPDATES = 1;
END $$;
DELIMITER ;
CALL ACTUALIZAR_MODULOS(0,19,3);


select * from ventas order by id_venta desc limit 30;
UPDATE ventas SET ESTADO = 1 WHERE ID_VENTA = 4139;

SELECT * FROM SUB_VISTAS ORDER BY ID DESC;
INSERT INTO SUB_VISTAS(NOMBRE, ID_AREA) VALUES ("GESTIONAR REGULARIZACION",4);

SELECT B.ID,B.NOMBRE AS SUB_VISTA, A.NOMBRE AS AREA FROM SUB_VISTAS AS B
LEFT JOIN AREAS AS A
ON B.ID_AREA = A.ID
WHERE A.NOMBRE = "FACTURACION"
;